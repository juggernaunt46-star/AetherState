...
def _load_evidence(
    ...
    if (
        ...
        or reason_code not in STABLE_REASON_CODES
    ):
        return _invalid_row(expected_id, evidence_commit)
    status_reason_valid = (
        gate_status_reason_is_valid(
...
def build_report(
    ...
    if not is_ancestor(repo, STAGE_3_MERGE_TARGET, evidence_commit):
        forced_reason = "evidence_not_ancestor"

    required_ids = set(REQUIRED_STAGE_3_GATES)
    seen_files: dict[str, Path] = {}
    if evidence_dir.is_dir():
        for path in sorted(evidence_dir.glob("*.json")):
            if path.stem not in required_ids or path.stem in seen_files:
    ...
    gates = [
        ...
        for gate_id in REQUIRED_STAGE_3_GATES
    ]
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print(f"{report['status']} {STAGE_3} {report['reason_code']}")
    return 0
