...
| turn_trace | `false` | structured local per-turn diagnostics. `Start-AetherState.bat` enables it; a direct/custom server start remains opt-in |
| turn_trace_max_mb | `16` | maximum active `turn-trace.jsonl` segment size before rotation |
| turn_trace_backups | `3` | rotated segments retained in addition to the active trace |
...
| reserve_lost_turns | `true` | RPG-only lost-delivery guard. When the immediately preceding turn has no assistant text/hash and the next request repeats the same Player action identity, re-serve its already-settled checks, raw rolls, enemy action, and empty mechanical disposition instead of consuming RNG or applying clock, costs, cooldowns, HP, or intent advancement again. It can recover the newest earlier same-hash settled turn across an empty duplicate turn. `false` restores fresh resolution; `specialization = none` remains inert. |
...
| live_recalc | true | Ingest each NEW normal reply on its own cold path: its R9/R10 world-tags commit and Tier-1 extraction flushes when the stream ends (formerly lag-1, where state reflected the reply before the latest one). A same-turn swipe retracts narrator extraction, marks that extraction range `skipped`, and stores replacement prose without tags, Tier-1, discovery, lint, genesis, evolution, or another state/mechanics settlement pass. Rule-owned checks, costs, HP, enemy action, and next intent survive. `false` = legacy lag-1 for normal turns; swipes remain narration-only. |
...
| `POST /aether/playerlex/propose` | test text in memory and return exact-span recognition candidates or stale/missing/corrupt refusals; never authorizes or executes |
| `GET /aether/player-lessons` | list explicit local narration and intent lessons with current/disabled/stale status, optimistic version proof, provenance, and dynamic anchor status; reads no chat/world prose |
| `GET /aether/player-lessons/selections?session_id=` | list latest content-free narration selection and upstream-delivery evidence, optionally restricted to one session's branches |
| `GET /aether/player-lessons/applications?session_id=` | list latest content-free intent application/non-application evidence after recognition and before contextual binding; read-only, with no public application-write route |
| `POST /aether/player-lessons` | save and enable one complete lesson; narration uses `do`/`avoid` plus an optional current PlayerLex anchor, while intent uses `misunderstanding`/`correct_interpretation` plus a required current ActionLex/action or ReferentLex/target anchor; returns 201 |
| `POST /aether/player-lessons/test` | test the selected type's complete unsaved definition plus `sample_text` and code narration mode in memory; returns `sample_stored=false`, stores no lesson/receipt/sample, and never invokes the narrator |
...
| `POST /aether/session/{sid}/worldlex/definitions` | append one exact `capability-definition/1` revision to the session world's immutable WorldLex store; storage is recognized only and returns `authorized=false, executable=false` |
| `POST /aether/session/{sid}/worldlex/assignments` | journal one exact `capability_assign` from `definition_ref`, domain-neutral `subject`, and `acquisition_source`; returns the baked assignment with `authorized=true, executable=false` |
...
| `GET /aether/session/{sid}/narrator-card.png` | download a **world-specific Narrator card** (PNG with embedded V2 chara card data — ST import format). It projects the preserved Creator-authored World + Player sources, not later live progression; legacy checkpoints safely fall back to reconstructed state. `Content-Disposition` uses the revisioned filename `<card-name>--<first-16-seed-hash>.png`. Read-only; a `none` session is unaffected (control route, off the relay) |
| `GET /aether/session/{sid}/narrator-card.json` | inspect the same V2 card as JSON. `data.extensions.aetherstate` carries `seed`, `seed_version`, `seed_fingerprint_version`, and the full `seed_fingerprint` (`sha256:<64 hex>`) |
| `POST /aether/session/{sid}/narrator-card` | build from that session's preserved Creator sources and atomically install the PNG when `[specialization].narrator_card_dir` is a real directory; returns `{name, world, bytes, installed, error, filename, seed_fingerprint, tags, seeded_world, seeded_player, png_b64, download}`. `seeded_world` / `seeded_player` report which documents the card embeds, not whether a session write occurred. Installation is best-effort and fail-open |
| `POST /aether/narrator-card` | **session-free** card build (2026-07-08): body `{world, player}` (the Creator forms) — canonicalizes and jointly validates the posted documents, requires no committed session, and atomically installs to `narrator_card_dir` when set. Returns `{name, world, world_id, bytes, installed, error, filename, seed_fingerprint, tags, seeded_world, seeded_player, png_b64}`. The structured portable source is embedded at `data.extensions.aetherstate.seed`; `seeded_world` / `seeded_player` describe its contents |
| `POST /aether/session/{sid}/seed` | **Atomic idempotent auto-seed:** body `{seed:{world, player}, seed_fingerprint:"sha256:..."}`. The supplied fingerprint must match the exact portable seed. World/Character operations and the durable successful-admission receipt commit in one transaction; any rejected or partial operation rolls the entire attempt back. The receipt has its own canonical integrity digest covering the raw card seed, normalized World/Character sources, requested flags, exact IDs, session/branch scope, admission counters, migration state, and commit time. An unsigned pre-integrity row grants no authority and is resealed only after a later exact admission re-proves current source postconditions. An exact concurrent/repeated request returns the committed receipt without replaying the write, while a different card fingerprint cannot borrow another seed's success. A transport failure, truncated response, or `5xx` is ambiguous and is reconciled through the read-only status route; a `409` source conflict or partial/stale admission and a `422` malformed request/fingerprint are terminal for that request. The ST extension calls this before genesis; deterministic, no LLM, privileged `source='user'` through `world_to_ops`/`player_to_ops` |
| `GET /aether/session/{sid}/seed-status?seed_fingerprint=sha256:...` | Read-only reconciliation for an ambiguous or lost `/seed` response. `404` means the exact session has not appeared yet and `202` means it exists but the exact receipt is not committed yet; both are pollable after an ambiguous POST. `409` means a different, stale, or damaged durable receipt and `422` means the fingerprint itself is invalid; both are terminal. A `200` response is complete only for the exact durable admission receipt. The route never mints a session, infers success from similar names, retries the write, or changes the ledger. `Cache-Control: no-store` |
...
Card revisions are content-addressed by the canonical embedded seed (creative-direction `notes` are excluded). The full `sha256:` value is returned as `seed_fingerprint`, and the installed/downloaded filename ends in its first 16 hexadecimal characters. Regenerating the exact same canonical World + Player source — including the same `world_id` — is idempotent: it yields the same fingerprint and filename and atomically replaces that one installed path. A changed portable source yields a different filename, so prior hashed revisions and legacy unhashed cards remain available. Revisions intentionally keep the same player-facing card name; when SillyTavern shows duplicate display names, choose the character whose backing filename exactly matches the Creator result before starting the new chat. This is additive to the world-agnostic `build_narrator_card.py` at the repo root (still the floor card for "no world yet").
...
World documents now carry one stable `world_id`. The world route commits it through
`world_identity_set`; a conflicting or invalid lineage rejects the entire world batch with HTTP 409
instead of salvaging that world's lore into the already-bound session. Definition, lineage, journal,
receipt, and checkpoint writes share one transaction boundary.

The Console **PlayerLex** tab is session-independent. It lists explicit local approval provenance and
current/stale/missing/corrupt status, supports optimistic correction/reapproval and secure local
removal, searches/pages all four Lexes without preloading the complete catalog, and tests recognition
proposals without saving the tested text. Opening the tab skips the shared session loader. On actual
new RPG Player turns, current matches enter the compiled LexFabric receipt once; they do not feed the HUD,
session ledger, WorldLex assignment, or mechanic path directly and cannot create a capability
candidate or check.

The Console **Player Lessons** tab is a separate session-independent management surface. The Player
chooses **Narration behavior** or **Intent interpretation**, supplies a complete definition, tests a
draft/sample in memory, and explicitly saves, revises, disables, re-enables, or securely removes it.
Narration asks what AetherState should do/avoid and may use any current PlayerLex anchor or none; the
Console discloses before save that a relevant selected title/`do`/`avoid` may be sent to the configured
model provider. Intent stores misunderstanding/correct-interpretation notes for the Player's record
only: AetherState never parses or sends those notes, and only the required current ActionLex anchor
for `action` or ReferentLex anchor for `target` can narrow a safe live ambiguity. Actor correction is
not offered. With a session open, the Console shows narration selection/upstream-delivery evidence
and intent application evidence separately.

The runtime selects both paths only on fresh actual RPG turns and freezes separate receipts even for
zero matches. Reserved swipe/Continue/edit-fork/lost-reply paths rehydrate narration without
reranking; they do not rerun or reapply intent. Service-level intent rehydration preserves
content-free selection/application evidence for inspection and ancestor cloning only. Exact
transport duplicates reuse the frozen enriched request/context only while its lesson revisions remain
current. Only narration can enter the separate five-lesson, 800-token prompt block, and a private
lesson block is never retained for prompt prewarm. Retention is not delivery, which is acknowledged
only after upstream transport returns response headers; that receipt proves neither narrator
adherence nor response completion. Intent is deterministic typed interpretation after recognition
and before contextual binding and never reaches the narrator. Both paths are inert under the RPG
semantic truth gate. Secure removal covers the active database/WAL and owned caches, but cannot recall
external backups or lesson text already received, retained, or in flight at a model provider. The
[public README](../README.md#player-lessons) summarizes the Player-facing contract.
...
|---|---|---|
| `name` | `"none"` | `none` \| `rpg` |
| `semantic_truth_gate` | `false` | Opt-in finite, proof-carrying narration delivery. It remains separate from the playable default while its bounded plan-selection path is promoted. |
| `narration_pre_display_guard` | `true` | RPG-only playable safety layer. A narrator reply with current code-settled mechanic authority is privately buffered, checked for definite roll-result or exact-combatant harm/death contradictions, and then released byte-exact when truthful or replaced by a code-authored safe result before display. Pure roleplay, background generations, explicit passthrough, upstream errors, and `name="none"` retain the ordinary transparent stream. |
| `intent_floor` | `true` | Pure-code natural-action detection for owned skills and present targets. It adds deterministic stemming and curated synonyms, ignores action/skill words inside quoted speech, and remains model/network-free; `false` keeps exact owned-name/`governs` matching. |
...
- **MAIN-LLM complete authoring** (`POST .../author`, cold-path, creation-time) — the player supplies
  ...
  **Completion contract (2026-07-17):** authoring runs at temperature 0.9 with a configurable
  ...
  field — including structured loot/fronts/routes — rides as canon context. The browser keeps the
  request-start snapshot and three-way rebases a slow response onto the current form: untouched
  paths may receive generated values, while newer Player edits win. Player custom definitions are
  canonicalized to the same `defs` shape before that comparison, so custom skills and abilities
  cannot be reported as preserved and then disappear. Persistent progress, elapsed time, failure,
  stale-context, and retry states remain visible; AI, deterministic defaults, presets, and committed
  session prefills share a single-flight gate so an overlapping paid request is never launched. The window also has
