...
- Added ClaimLex recognition and typed Claim Records that keep speech, attribution, belief,
  knowledge, doubt, rumor, accepted facts, and admitted world truth separate.
- Added immutable World Event Records and active WorldOverlay projections for supported world,
  location, actor, NPC, eligibility, quest, faction, relationship, reputation, retrieval,
  briefing, narration, Console, and HUD effects, including expiry, reversal, and supersession.
- Made Claim/Event storage branch-aware, replayable, idempotent, fork-safe, and privacy-aware;
  recognition and narrator prose still cannot establish objective truth or admit an event.
- Rebuilt the sealed Semantic Atlas to 327 meanings: 265 CapabilityLex, 13 ReferentLex,
  18 SceneLex, 15 ActionLex, and 16 ClaimLex.
- Overhauled Creator authoring: Creative Direction is sent as controlling instruction, MAIN model
  output defaults to 32,768 tokens and can be configured to 131,072, truncated documents are
  rejected and retried once, and generated fields survive card/session round trips.
- Expanded Creator, Console, HUD, and SillyTavern presentation for fronts, routes, loot, custom
  skill/ability definitions, gear effects, claims, epistemics, facts, and event history.
- Fixed entity-name collisions, stale/forged semantic anchors, duplicate overlay cells,
  cross-session event forks, future-subject leakage, and dead-helper startup waits.

## 1.22.0 — 2026-07-17

This is the cumulative public sync of AetherState's current local-first RPG and semantic foundation.
It keeps the central contract intact: the model narrates, while code-owned mechanics and the Ledger
remain the authority for outcomes and world truth.

- **Player Lessons.** The Console now lets a Player explicitly save, test, inspect, revise, disable,
  re-enable, and securely remove two bounded lesson types: narration-behavior preferences and safe
  action/target intent corrections. Nothing is mined from chats. Retrieval is relevance-based on a
  real new RPG turn, stale semantic anchors fail closed, and retry/swipe/Continue/replay/duplicate
  paths do not relearn or reapply a lesson. Narration instructions may reach the configured narrator
  provider only after visible consent; intent notes remain local and are never sent to the narrator.
- **PlayerLex and the Semantic Atlas.** Players can approve local names, aliases, and anchored
  authoring patterns against one exact current meaning. The sealed Atlas exposes 311 meanings across
  CapabilityLex, ReferentLex, SceneLex, and ActionLex. PlayerLex participates once in real new-turn
  RPG recognition, but remains recognition-only: it cannot grant a capability, settle an outcome,
  or create world truth.
- **Grounded enemies and semantic causality.** The current source includes frozen enemy kits,
  deterministic intent/replay plumbing, semantic binding and occurrence records, narrator transfer,
  pre-display truth checks, and stronger turn-lifecycle fencing. Generated enemy effects remain
  deliberately bounded by implemented receipts; names and lore do not become mechanics by prose.
- **Player visibility and local control.** The Console and SillyTavern companion expose the new
  PlayerLex and Player Lessons controls, provenance, current/stale state, test results, and
  content-free selection/application evidence. The extension manifest now tracks the package
  release and links back to the public repository.
- **Privacy and removal boundaries.** Player-approved definitions stay in the local AetherState
  database. Draft samples are not stored. Secure removal clears the owned active-database, WAL, and
  process-cache copies, while honestly not claiming to recall external backups or text a configured
  provider already received.
- **Verification.** The release candidate is covered by the current backend, semantic, lifecycle,
  Console, SillyTavern, retry/replay, and privacy regression suites. The semantic truth gate remains
  off by default; this update does not claim AoE settlement, unrestricted world-event authorship, or
  authority from learned text.

## 1.21.0 — 2026-07-11

Large-scale battle — you fight your corner of it on the dice while the whole battle rages in prose,
and the fight keeps coming until the tide turns—the original macro-battle vision, built on the War Room.
...
a crowd. Closes the former "3v3 is missing" gap.
...
- **Battlefield allies — enemy of my enemy.** In an ambush, whoever is fighting the same foes and
  ...
  faction allies. For example, a merchant caravan's sellswords all fight the bandits to survive;
...
The Creator auto-fill finally completes the boxes (a long-standing bug, live-reproduced
...
- `[clash | A vs B | how | outcome]` + a `clash_record` extraction op: NPC-vs-NPC fights
  stay prose (no dice) but method + outcome commit as facts on REAL rows.
...
- Outside an open bracketed intent like `[I persuade Jerald.]`, the DM deciding for your
  character ("The Player agrees to the terms…") is flagged and corrected next turn. Inside one,
