...
# ------------------------------ (c) malformed authored rows ------------------------------
def test_gear_repairs_dangling_separator_and_preserves_valid_punctuation():
    out = creator._norm_gear([
        "weatherproof field coat —",
        "Smith & Wesson M&P 2.0",
        "Old Guard's key-ring",
    ])
    assert out == ["weatherproof field coat", "Smith & Wesson M&P 2.0",
                   "Old Guard's key-ring"]


def test_gear_rejects_obviously_incomplete_name_rows():
    out = creator._norm_gear([
        "ledger hook (for",
        'brass token "marked',
        "coil of",
    ...
    assert out == ["complete notebook"]


def test_structured_gear_keeps_item_but_drops_malformed_optional_effect():
    out = creator._norm_gear([{
        "name": "harbor badge:",
        "slot": "accessory1",
        "effect": "identifies the wearer as",
    }])
    assert out == [{"name": "harbor badge", "slot": "accessory1"}]
