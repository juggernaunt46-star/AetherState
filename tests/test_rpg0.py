...
def test_overlay_precedence_user_beats_profile_beats_default(tmp_path):
    ...
    assert c_profile.specialization.semantic_truth_gate is False      # opt-in until plan promotion
    assert c_profile.specialization.narration_pre_display_guard is True
    ...
    p.write_text('[specialization]\nname = "rpg"\nsemantic_truth_gate = false\n'
                 'narration_pre_display_guard = false\n'
                 '[injection.priorities]\nquest = 77\n')
    c_user = load_config(p)
    assert c_user.specialization.semantic_truth_gate is False         # explicit user override
    assert c_user.specialization.narration_pre_display_guard is False   # user beats profile
...
def test_specialization_env_and_default(tmp_path, monkeypatch):
    ...
    assert Config().specialization.semantic_truth_gate is False
    assert Config().specialization.narration_pre_display_guard is True
    ...
    assert c.specialization.semantic_truth_gate is False
    assert c.specialization.narration_pre_display_guard is True
...
def test_player_seed_resource_metadata_replacement_and_partial_omission():
    st = empty_state()
    st["entities"]["kael"] = {"kind": "character", "name": "Kael", "present": True}
    reduce_state(st, [{"op": "player_seed", "entity": "kael", "card": {
        "hp": {"cur": 19, "max": 30},
        "resources": {
            "Ash Focus": {"name": "Ash Focus", "cur": 4, "max": 8,
                          "color": "#b56cff"},
            "stamina": {"cur": 5, "max": 12},
        },
    }, "_turn": 0}])
    rec = st["player"]["kael"]
    assert rec["hp"] == {"cur": 19, "max": 30}
    assert rec["resources"]["ash_focus"] == {
        "cur": 4, "max": 8, "name": "Ash Focus", "color": "#b56cff",
    }

    reduce_state(st, [{"op": "player_seed", "entity": "kael", "card": {
        "resources": {"resolve": {"name": "Resolve", "cur": 2, "max": 6,
                                    "color": "not-a-color"}},
    }, "_turn": 1}])
    rec = st["player"]["kael"]
    assert rec["hp"] == {"cur": 19, "max": 30}       # resources omission does not touch HP
    assert rec["resources"] == {"resolve": {"cur": 2, "max": 6, "name": "Resolve"}}

    reduce_state(st, [{"op": "player_seed", "entity": "kael",
                       "card": {"level": 8}, "_turn": 2}])
    assert st["player"]["kael"]["resources"] == {
        "resolve": {"cur": 2, "max": 6, "name": "Resolve"},
    }                                                               # partial seed leaves pools


def test_complete_player_seed_replaces_omitted_frozen_capabilities():
    """Creator's full ownership snapshot cannot retain an older custom def as roll authority."""
    from aetherstate import registry

    st = empty_state()
    st["entities"]["kael"] = {"kind": "character", "name": "Kael", "present": True}
    reduce_state(st, [{"op": "player_seed", "entity": "kael", "card": {
        "skills": {"ashbind": 2},
        "abilities": ["ember_lunge"],
        "resources": {"cinder": {"cur": 4, "max": 4}},
        "defs": {
            "skills": {"ashbind": {
                "name": "Ashbind", "keyed_stat": "CUN", "cost": {"cinder": 2},
            }},
            "abilities": {"ember_lunge": {
                "name": "Ember Lunge", "kind": "active", "mechanic": "surge",
                "cost": {"cinder": 2},
            }},
        },
    }, "_turn": 0}])
    assert registry.load().resolve_skill("ashbind", st["player"]["kael"]) == "ashbind"

    # This is the normal Creator shape: both ownership tables are present, while omitted defs and
    # pools mean "remove them", not "merge with the previous card".
    reduce_state(st, [{"op": "player_seed", "entity": "kael", "card": {
        "skills": {"perception": 1},
        "abilities": [],
        "resources": {"stamina": {"cur": 8, "max": 8}},
    }, "_turn": 1}])
    rec = st["player"]["kael"]
    assert "defs" not in rec
    assert rec["_resource_cost_policy"] == "strict/1"
    assert rec["skills"] == {"perception": 1} and rec["abilities"] == []
    assert rec["resources"] == {"stamina": {"cur": 8, "max": 8}}
    assert registry.load().resolve_skill("ashbind", rec) is None
    assert not registry.load().has_ability(rec, "ember_lunge")


def test_partial_player_seed_keeps_legacy_frozen_definition_merge():
    """A hand-authored partial patch remains a merge unless both ownership tables are supplied."""
    st = empty_state()
    st["entities"]["kael"] = {"kind": "character", "name": "Kael", "present": True}
    reduce_state(st, [{"op": "player_seed", "entity": "kael", "card": {
        "defs": {"skills": {"old_lore": {"name": "Old Lore", "keyed_stat": "CUN"}}},
    }, "_turn": 0}])
    reduce_state(st, [{"op": "player_seed", "entity": "kael", "card": {
        "level": 3,
        "defs": {"skills": {"new_lore": {"name": "New Lore", "keyed_stat": "INT"}}},
    }, "_turn": 1}])
    assert set(st["player"]["kael"]["defs"]["skills"]) == {"old_lore", "new_lore"}
...
def test_dm_contract_v2_in_fiction_and_known_vs_present():
    ...
    cfg = Config()
    cfg.specialization.name = "rpg"
    combat_contract = rules_contract(cfg)
    assert "not a reset, hesitation, or skipped enemy turn" in combat_contract
    assert "Player's one response window" in combat_contract
    assert "settles THIS attempt NOW" in DM_RULES_CONTRACT   # RPG-5: no directive-dodging
    assert "You NEVER request, emit, simulate, or arm a roll" in DM_RULES_CONTRACT
    assert "AetherState alone decides and resolves checks" in DM_RULES_CONTRACT
    assert "already-committed damage exactly" in DM_RULES_CONTRACT
    assert "never emit an [hp] tag for it" in DM_RULES_CONTRACT
    assert "What will you do?" in DM_RULES_CONTRACT
    assert "KNOWN, not on-scene" in DM_RULES_CONTRACT
    assert "aether.check" not in DM_RULES_CONTRACT
