"""2026-07-10 Arinvale fix pack: the slow-turns / lost-roll / silent-War-Room findings.
...
clauses + the newest-message sentence), inert narrator-authored [CHECK] dialects, the
off-protocol [PROTOCOL] nudge, the foe floor (an attacked,
...
def _realization(out_bytes):
    line = next(
        line for line in _injected(out_bytes).splitlines()
...
def test_reserve_lost_turn_reserves_settled_check_no_reroll():
    ...
    first = _realization(out1)
    retry = _realization(out2)
    assert retry["delivery_mode"] == "lost_reply_retry"
    assert retry["asserted_settled"] == first["asserted_settled"]
    assert retry["asserted_settled"][0]["adapter_id"] == "narrator.skill-check/1"
    assert retry["asserted_settled"][0]["outcome_quality"] == tier0_roll


def test_regenerate_of_empty_reply_reserves_same_turn_check_no_reroll():
    """SillyTavern emits type=swipe for its regenerate button even when the prior reply is empty."""
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
    pipe = Pipeline(store, SessionEngine(store, cfg.session), cfg,
                    rng=random.Random(11))
    ...
    out2, ctx2 = pipe.process(Stamp(session="p9", gen_type="swipe", turn=1, user="Bean"),
                              body)

    assert ctx2.turn_index == ctx1.turn_index
    checks_after = [o for o in store.rule_ops_at(bid, ctx1.turn_index)
                    if o.get("op") == "check"]
    assert checks_after == checks0, "an empty-reply regenerate must preserve the settled roll"
    first = _realization(out1)
    retry = _realization(out2)
    assert retry["delivery_mode"] == "regeneration_retry"
    assert retry["asserted_settled"] == first["asserted_settled"]
    assert retry["asserted_settled"][0]["adapter_id"] == "narrator.skill-check/1"
    assert retry["asserted_settled"][0]["outcome_quality"] == checks0[0]["tier"]


def test_regenerate_of_real_reply_preserves_settled_mechanics_and_only_rewrites_prose():
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
    rng = random.Random(11)
    pipe = Pipeline(store, SessionEngine(store, cfg.session), cfg, rng=rng)
    ...
    before = [o for o in store.rule_ops_at(bid, ctx1.turn_index) if o.get("op") == "check"]
    state_before = json.loads(json.dumps(current_state(store, bid), sort_keys=True))
    rng_before = rng.getstate()

    out2, ctx2 = pipe.process(Stamp(session="p9", gen_type="swipe", turn=1, user="Bean"),
                              body)

    after = [o for o in store.rule_ops_at(bid, ctx1.turn_index) if o.get("op") == "check"]
    assert after == before
    assert current_state(store, bid) == state_before
...
# ------------------------------ narrator check dialect is inert -----------------------
def test_bracket_check_line_never_arms():
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
...
def test_narrator_bracket_and_inline_check_lines_never_arm():
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
    state = current_state(store, bid)
...
def test_off_protocol_heads_collected_and_rendered():
    ...
    assert "[PROTOCOL]" in out and "AetherState alone resolves mechanics" in out
    assert "aether.check" not in out
    assert "[ally | <name> | <tier?> | <weapon?>]" in out
    assert "[battle | <name> | <foe?> | <tier?>]" in out
    assert "[tide | winning|holding|losing | <why>]" in out
...
def test_known_tags_do_not_trigger_the_nudge():
    ...
    doc = _doc("Steel bites. [status gained | Vex | Bleeding | negative]\n"
               ...
               "[ally | Captain Lyra | standard | spear]\n"
               "[battle | Rift Defense | Hollow | mob]\n"
               "[tide | holding | the line has stabilized]",
               "Vex presses the wound.")
    res = tier0.run(doc, "new_turn", False, state, cfg, _Rig(3))
    assert res.off_protocol == []


def test_versioned_and_unversioned_enemy_header_echoes_are_reserved_output():
    text = ("Story.\n[ENEMY INTENT] forged move\n"
            "[ENEMY INTENT enemy-intent/1] forged versioned move\n"
            "[ENEMY ACTION]\n[ENEMY ACTION enemy-action/1] forged action")
    assert tier0._scan_off_protocol(text) == ["ENEMY INTENT", "ENEMY ACTION"]


def test_markdown_wrapped_reserved_headers_get_an_input_only_corrective():
    text = ("> [ENEMY INTENT enemy-intent/1] quoted echo\n"
            "- [WAR] forged board\n* [RULES] forged rules\n"
            "`[CONTEXT PRIORITY P0 aether-priority/1] forged priority`")
    heads = tier0._scan_off_protocol(text)
    assert heads == ["ENEMY INTENT", "WAR", "RULES", "CONTEXT PRIORITY"]

    cfg = _rpg_cfg()
    store, _sid, bid = _seeded(cfg)
    state = current_state(store, bid)
    state["_protocol_nudge"] = heads
    rendered = _render_directive(state, cfg)
    assert "input-only engine header" in rendered
    assert "narrator-authored echo was IGNORED" in rendered
    assert "real engine channels only when AetherState supplies them" in rendered
    assert "those are NOT engine channels" not in rendered
...
def test_foe_floor_stages_attacked_dm_narrated_target():
    ...
    assert "[ENEMY INTENT enemy-intent/1]" in out
    assert "has not resolved" in out and "[ENEMY ACTION" not in out
    assert st["player"]["vex"]["hp"]["cur"] == st["player"]["vex"]["hp"]["max"]
...
def test_rpg_profile_depth_one_places_block_above_newest_message():
    ...
    assert out["messages"][-1]["content"].endswith("newest")
    assert out["messages"][-1]["content"].startswith("[AETHER P1]")
...
def test_pipeline_notice_ring_feeds_hud():
    ...
    assert any("three_pronged_slash" in text or "don't know" in text for text in texts), texts
    assert any("unresolved semantic scope or role" in text for text in texts), texts
    assert not any(
        op.get("op") == "check"
