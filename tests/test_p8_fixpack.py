...
(player explicit/NL checks still work; none stays inert; replay pure),
...
def test_opposition_die_renders_and_is_deterministic():
    cfg = _rpg_cfg()
    cfg.specialization.war_room = False       # legacy generic-opposition compatibility lane
    store, sid, bid = _seeded(cfg)
    apply_delta(store, sid, bid, 1, [
    ...
    st = current_state(store, bid)
    st["meta"]["turn"] = 4
    opp = tier0._opposition_op(st, cfg, turn=4)
    assert opp is not None
    apply_delta(store, sid, bid, 4, [opp], "rule", cfg)
    st = current_state(store, bid)
    ...
    opp2 = tier0._opposition_op(st, cfg, turn=5)
    assert opp2 is not None
    apply_delta(store, sid, bid, 5, [opp2], "rule", cfg)
    st = current_state(store, bid)
    assert "[OPPOSITION]" in _render_directive(st, cfg)
    assert "do not emit an [hp] tag" in _render_directive(st, cfg)


def test_opposition_miss_cannot_spend_an_unstated_player_dodge():
    cfg = _rpg_cfg()
    store, _sid, bid = _seeded(cfg)
    st = current_state(store, bid)
    st["meta"]["turn"] = 4
    st["player"]["vex"]["_opposition_last"] = {
        "turn": 4, "total": 5, "tier": "MISSES", "damage": 0,
    }
    directive = _render_directive(st, cfg)
    assert "The miss belongs to the enemy" in directive
    assert "Do not make the Player dodge, parry, block, twist" in directive


def test_opposition_absent_without_present_hostile_or_under_none():
    cfg = _rpg_cfg()
    cfg.specialization.war_room = False       # this test owns the legacy arming rules
    store, sid, bid = _seeded(cfg)
    st = current_state(store, bid)                    # no one else present
    assert tier0._opposition_op(st, cfg, turn=2) is None
    ...
    assert tier0._opposition_op(st_f, cfg, turn=2) is None
    st_c = current_state(store, bid)                  # combat phase arms it without affinity
    st_c.setdefault("scene", {})["phase"] = "climax"
    opp = tier0._opposition_op(st_c, cfg, turn=2)
    assert opp is not None
    apply_delta(store, sid, bid, 2, [opp], "rule", cfg)
    assert "[OPPOSITION]" in _render_directive(current_state(store, bid), cfg)
    cfg2 = _rpg_cfg()
    cfg2.specialization.war_room = False
    cfg2.specialization.enemy_rolls = False           # the knob turns it off
    st2 = current_state(store, bid)
    st2.setdefault("scene", {})["phase"] = "climax"
    assert tier0._opposition_op(st2, cfg2, turn=3) is None
    ...
    assert tier0._opposition_op(st2, none_cfg, turn=3) is None


# ------------------------------ narrator check calls are inert -------------------------
...
def test_narrator_inline_check_never_arms_on_plain_prose():
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
...
def test_narrator_multiword_check_never_arms():
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
    state = current_state(store, bid)
...
def test_player_explicit_check_is_independent_of_narrator_syntax():
    cfg = _rpg_cfg()
    store, sid, bid = _seeded(cfg)
    state = current_state(store, bid)
...
def test_narrator_check_none_session_inert():
...
def test_legacy_dm_called_provenance_remains_renderable():
    state = {"meta": {"turn": 3},
    ...
    assert "legacy pre-settled engine check" in out
...
def test_coerce_infers_mechanic_from_effect_text():
    ...
    assert "cost" not in ab["dead_man_switch"]           # blank in Creator means no cost
