...
def test_mod_active_applies_on_use_pays_and_cools():
    ...
    assert base["_shape"]["schema"] == "check-roll-shape/1"
    assert base["_shape"]["burst"] == 0              # never auto-applies
    assert base["_shape"]["abilities"] == []
    assert base["_shape"]["executed_active_ids"] == []
...
def test_mod_active_respects_cooldown():
    ...
    assert op["_shape"]["burst"] == 0                 # the burst did NOT land
    assert op["_shape"]["abilities"] == []
